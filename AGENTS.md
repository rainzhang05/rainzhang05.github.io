...
- **The entrance is withdrawn, not paused.** `animation-play-state: paused` is the obvious way to hold `.enter` and it does not work: an animation held from before it ever started never resumes in Chrome — the play state goes back to `running` and the current time stays at zero for good. The rule at the foot of `globals.css` uses `animation: none` for `pending` and `leaving`, and says nothing at all about `done`, so the replay on a locale change still happens.
