  ...
  featured: [
    {
      ...
      sections: [
        {
          label: '概要',
          text: 'MNT Realty の業務基盤です。見込み客向けの公開ウェブサイト、同社が管理するストラータの居住者がお知らせを読み、共用設備を予約し、書類を取得するオーナーポータル、そしてスタッフがそのすべてを運用する管理コンソール。従来は 553 KB の HTML 1枚が4種類の利用者を CSS で切り替え、すべてを1つの PHP エンドポイントで処理していました。',
        },
        {
          label: '仕組み',
          text: '3つの画面、3つのサブドメイン、ビルドは1つです。プロキシがホスト名で振り分け（admin.* はコンソール、owners.* はポータル）、公開ホストではそれらのパスを not-found にします。画面ごとに独自のクロームと、ホスト限定のセッション Cookie を持ちます。公開ページは事前生成し、コンソールとポータルはリクエストごとに描画します。読み書きはすべて1つのデータストア契約を通り、書き込みは1操作、読み出しは画面ごとに1モデル。スタッフの入力はそのまま居住者に届き、背後のストアは画面に触れずに、設計済みの PostgreSQL スキーマへ差し替えられます。',
        },
        {
          label: '技術的な課題',
          text: 'アクセス制御をガードごとではなく1か所で決めること。ナビゲーション、ページのガード、ファイル配信はすべて同じ表を読み、権限外のセクションは「禁止」ではなく「存在しない」として扱います。テナントにオーナーの領域の存在を知らせないためです。ストラータを本当の境界にし、あるストラータを開いている間は別のストラータの情報に一切たどり着けないようにすること。そして会社情報・書類手数料表・共用設備の枠テンプレートといった業務上の事実を、ソース中のリテラルではなく、管理者が設定画面で編集できるバージョン管理付きのデータとして扱い、保存したどの版にも戻せるようにすることです。',
        },
      ],
      image: {
    ...
    },
    {
      ...
      sections: [
        {
          label: '概要',
          text: '飛天ジャパンの顧客と営業チームのためのデモです。開発者向けプラットフォームがエンジニア向けなのに対し、こちらは製品を検討している人向けです。仕様書を読まなくても、セキュリティキーを登録し、サインインし、耐量子計算機暗号の資格情報が動く様子を確かめられます。',
        },
        {
          label: '仕組み',
