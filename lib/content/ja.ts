  ...
  featured: [
    {
      ...
      sections: [
        {
          label: '概要',
          text: 'MNT Realty の業務基盤です。見込み客向けの公開ウェブサイト、同社が管理するストラータの居住者がお知らせを読み、共用設備を予約し、書類を取得するオーナーポータル、そしてスタッフがそのすべてを運用する管理コンソール。',
        },
        {
          label: '仕組み',
          text: '3つの画面、3つのサブドメイン、ビルドは1つです。プロキシがサブドメインで振り分け、admin はコンソールへ、owners はポータルへ送り、公開サイトでは同じパスを not-found にします。画面ごとに独自のクロームと、ホスト限定のセッション Cookie を持ちます。公開ページは事前生成し、コンソールとポータルはリクエストごとに描画します。読み書きはすべて1つのデータストア契約を通り、書き込みは1操作、読み出しは画面ごとに1モデル。スタッフの入力はそのまま居住者に届き、背後のストアは画面に触れずに、設計済みの PostgreSQL スキーマへ差し替えられます。',
        },
        {
          label: '技術的な課題',
